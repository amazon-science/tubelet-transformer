...
class PerImageEvaluation(object):
  ...
  def compute_object_detection_metrics(
    ...
    groundtruth_boxes, groundtruth_class_labels, groundtruth_is_difficult_list, groundtruth_is_group_of_list, groundtruth_masks = (
        self._remove_invalid_gtboxes(groundtruth_boxes, groundtruth_class_labels, groundtruth_is_difficult_list, groundtruth_is_group_of_list, groundtruth_masks))
  ...
  def _compute_tp_fp_for_single_class(
    ...
    # # added by JJ: set unmatched detections outside of area range to ignore
    vaild_id = []

    b_w = detected_boxes[:,2]-detected_boxes[:,0]
    b_h = detected_boxes[:,3]-detected_boxes[:,1]
    b_a = 1.306*b_w*b_h
    for i in range(num_detected_boxes):
      if tp_fp_labels[i] or (b_a[i]>self.area[0] and b_a[i]<self.area[1]):
        vaild_id.append(i)
    scores = scores[vaild_id]
    tp_fp_labels = tp_fp_labels[vaild_id]
    is_matched_to_difficult_box = is_matched_to_difficult_box[vaild_id]
    is_matched_to_group_of_box = is_matched_to_group_of_box[vaild_id]
  ...
  def _remove_invalid_gtboxes(self, groundtruth_boxes, groundtruth_class_labels, groundtruth_is_difficult_list, groundtruth_is_group_of_list, groundtruth_masks):
    """Removes entries with invalid boxes.

    A box is invalid if the box area is beyond the required box area.
    ...
    for j in range(g_a.shape[0]):
      ...
      if g_a[j]>self.area[0] and g_a[j]<self.area[1]:
        valid_indices.append(j)

    groundtruth_boxes = groundtruth_boxes[valid_indices]
    groundtruth_class_labels = groundtruth_class_labels[valid_indices]
    groundtruth_is_difficult_list = groundtruth_is_difficult_list[valid_indices]
    groundtruth_is_group_of_list = groundtruth_is_group_of_list[valid_indices]

    if groundtruth_masks is not None:
      groundtruth_masks = groundtruth_masks[valid_indices]
    return [
        groundtruth_boxes, groundtruth_class_labels, groundtruth_is_difficult_list, groundtruth_is_group_of_list, groundtruth_masks
    ]
